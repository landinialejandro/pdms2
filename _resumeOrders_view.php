...
// Download AppGini for free from https://bigprof.com/appgini/download/
	...
	$x->QueryFieldsTV = array(   
		...
		"`SQL_resumeOrders`.`TOT`" => "TOT",
		"`SQL_resumeOrders`.`MONTH`" => "MONTH",
		"`SQL_resumeOrders`.`YEAR`" => "YEAR",
		"`SQL_resumeOrders`.`DOCs`" => "DOCs",
		"IF(    CHAR_LENGTH(`orders1`.`id`), CONCAT_WS('',   `orders1`.`id`), '') /* Related */" => "related",
		"`SQL_resumeOrders`.`id`" => "id"
	);
	...
	$x->QueryFrom = "`SQL_resumeOrders` LEFT JOIN `kinds` as kinds1 ON `kinds1`.`code`=`SQL_resumeOrders`.`kind` LEFT JOIN `companies` as companies1 ON `companies1`.`id`=`SQL_resumeOrders`.`company` LEFT JOIN `kinds` as kinds2 ON `kinds2`.`code`=`SQL_resumeOrders`.`typedoc` LEFT JOIN `companies` as companies2 ON `companies2`.`id`=`SQL_resumeOrders`.`customer` LEFT JOIN `orders` as orders1 ON `orders1`.`id`=`SQL_resumeOrders`.`related` ";
	$x->QueryWhere = '';
	$x->QueryOrder = '';
	...
	if($perm[2]==1 || ($perm[2]>1 && $DisplayRecords=='user' && !$_REQUEST['NoFilter_x'])){ // view owner only
		...
		$x->QueryWhere="where `SQL_resumeOrders`.`id`=membership_userrecords.pkValue and membership_userrecords.tableName='_resumeOrders' and lcase(membership_userrecords.memberID)='".getLoggedMemberID()."'";
	}elseif($perm[2]==2 || ($perm[2]>2 && $DisplayRecords=='group' && !$_REQUEST['NoFilter_x'])){ // view group only
		$x->QueryFrom.=', membership_userrecords';
		$x->QueryWhere="where `SQL_resumeOrders`.`id`=membership_userrecords.pkValue and membership_userrecords.tableName='_resumeOrders' and membership_userrecords.groupID='".getLoggedGroupID()."'";
	}elseif($perm[2]==3){ // view all
		// no further action
	}elseif($perm[2]==0){ // view none
		$x->QueryFields = array("Not enough permissions" => "NEP");
		$x->QueryFrom = '`SQL_resumeOrders`';
		$x->QueryWhere = '';
		$x->DefaultSortField = '';
	}
