<?php
	...
	if($thisMD5==$prevMD5){
		...
		setupTable('vatRegister', "create table if not exists `vatRegister` (   `id` INT unsigned not null auto_increment , primary key (`id`), `idCompany` INT unsigned null , `companyName` INT unsigned null , `tax` VARCHAR(40) null default '4%' , `month` VARCHAR(40) null , `year` VARCHAR(40) null default '2018' , `amount` DECIMAL(10,2) null ) CHARSET utf8", $silent);
		setupIndexes('vatRegister', array('idCompany'));
		setupTable('companies', "create table if not exists `companies` (   `id` INT unsigned not null auto_increment , primary key (`id`), `kind` VARCHAR(40) not null , `kind_code` VARCHAR(40) null , `companyCode` VARCHAR(255) null , unique `companyCode_unique` (`companyCode`), `companyName` VARCHAR(255) null , `fiscalCode` VARCHAR(255) null , `vat` VARCHAR(255) not null , `notes` TEXT null , `codiceDestinatario` VARCHAR(7) null , `regimeFiscale` VARCHAR(40) null , `tipoCassa` VARCHAR(40) null , `modalitaPagamento` VARCHAR(40) null , `RiferimentoAmministrazione` VARCHAR(40) null , `FormatoTrasmissione` VARCHAR(40) not null default 'FPR12' , `REA_Ufficio` VARCHAR(2) null , `REA_NumeroREA` VARCHAR(20) null , `REA_CapitaleSociale` VARCHAR(40) null , `REA_SocioUnico` VARCHAR(2) not null default 'SU' , `REA_StatoLiquidazione` VARCHAR(2) not null default 'LN' , `RIT_soggettoRitenuta` VARCHAR(40) null default '0' , `RIT_tipoRitenuta` VARCHAR(40) not null default 'RT02' , `RIT_AliquotaRitenuta` DECIMAL(10,2) null , `RIT_CausalePagamento` VARCHAR(40) null ) CHARSET utf8", $silent, array( " ALTER TABLE `companies` CHANGE `codiceDestinatario` `codiceDestinatario` VARCHAR(7) null "," ALTER TABLE `companies` CHANGE `codiceDestinatario` `codiceDestinatario` TEXT null "," ALTER TABLE `companies` CHANGE `codiceDestinatario` `codiceDestinatario` VARCHAR(40) null "," ALTER TABLE `companies` CHANGE `codiceDestinatario` `codiceDestinatario` VARCHAR(7) null "));
		setupIndexes('companies', array('kind','regimeFiscale','tipoCassa','modalitaPagamento'));
		...
		setupTable('addresses', "create table if not exists `addresses` (   `id` INT unsigned not null auto_increment , primary key (`id`), `kind` VARCHAR(40) null , `address` VARCHAR(255) not null , `houseNumber` VARCHAR(255) not null , `country` INT unsigned not null , `country_name` INT unsigned null , `town` INT unsigned not null , `postalCode` INT unsigned not null , `district` INT unsigned not null , `contact` INT unsigned null , `company` INT unsigned null , `map` VARCHAR(40) null , `default` INT null default '0' , `ship` INT null default '0' ) CHARSET utf8", $silent);
		...
		setupTable('mails', "create table if not exists `mails` (   `id` INT unsigned not null auto_increment , primary key (`id`), `kind` VARCHAR(40) null , `mail` VARCHAR(255) null , `contact` INT unsigned null , `company` INT unsigned null ) CHARSET utf8", $silent);
		setupIndexes('mails', array('kind','contact','company'));
		setupTable('contacts_companies', "create table if not exists `contacts_companies` (   `id` INT unsigned not null auto_increment , primary key (`id`), `contact` INT unsigned null , `company` INT unsigned null , `default` VARCHAR(40) null default '0' ) CHARSET utf8", $silent);
