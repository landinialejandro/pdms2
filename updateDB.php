<?php
	...
	if($thisMD5==$prevMD5){
		...
		setupTable('_resumeOrders', "create table if not exists `_resumeOrders` (   `kind` VARCHAR(40) , `company` INT unsigned , `typedoc` VARCHAR(40) , `customer` INT unsigned , `TOT` VARCHAR(40) , `MONTH` VARCHAR(40) , `YEAR` VARCHAR(40) , `DOCs` VARCHAR(40) , `realted` INT unsigned , `id` INT unsigned not null auto_increment , primary key (`id`)) CHARSET utf8", $silent, array( "ALTER TABLE `_resumeOrders` CHANGE `cusotmer` `customer` INT unsigned "));
		setupIndexes('_resumeOrders', array('kind','company','typedoc','customer','realted'));
