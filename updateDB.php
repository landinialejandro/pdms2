<?php
	...
	if($thisMD5==$prevMD5){
		...
		// set up tables
		setupTable('orders', "create table if not exists `orders` (   `id` INT unsigned not null auto_increment , primary key (`id`), `kind` VARCHAR(40) not null , `progressiveNr` CHAR(40) , `trasmissionFor` CHAR(10) default 'SDI10' , `consigneeID` CHAR(10) , `company` INT unsigned not null , `typeDoc` VARCHAR(40) not null , `multiOrder` INT not null , `customer` INT unsigned , `employee` INT unsigned , `date` DATE , `requiredDate` DATE , `shippedDate` DATE , `shipVia` INT unsigned , `Freight` DECIMAL(10,2) , `pallets` INT , `licencePlate` VARCHAR(40) , `orderTotal` DECIMAL(10,2) , `cashCredit` VARCHAR(255) default '1' , `trust` INT unsigned , `overdraft` INT , `commisionFee` INT , `consigneeHour` DATETIME , `consigneePlace` VARCHAR(255) , `related` INT ) CHARSET utf8", $silent, array( " ALTER TABLE `orders` CHANGE `kind` `kind` VARCHAR(40) not null "," ALTER TABLE `orders` CHANGE `company` `company` INT unsigned not null "," ALTER TABLE `orders` CHANGE `typeDoc` `typeDoc` VARCHAR(40) not null "," ALTER TABLE `orders` CHANGE `multiOrder` `multiOrder` INT not null "));
