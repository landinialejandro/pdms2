...
// Data functions (insert, update, delete, form) for table _ordersSummary

// This script and data application were generated by AppGini 5.74
// Download AppGini for free from https://bigprof.com/appgini/download/

function _ordersSummary_insert(){
	global $Translation;

	// mm: can member insert record?
	$arrPerm=getTablePermissions('_ordersSummary');
	if(!$arrPerm[1]){
		return false;
	}
	...
	// hook: _ordersSummary_before_insert
	if(function_exists('_ordersSummary_before_insert')){
		$args=array();
		if(!_ordersSummary_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o = array('silentErrors' => true);
	sql('insert into `_ordersSummary` set       `kind`=' . (($data['kind'] !== '' && $data['kind'] !== NULL) ? "'{$data['kind']}'" : 'NULL') . ', `company`=' . (($data['company'] !== '' && $data['company'] !== NULL) ? "'{$data['company']}'" : 'NULL') . ', `typedoc`=' . (($data['typedoc'] !== '' && $data['typedoc'] !== NULL) ? "'{$data['typedoc']}'" : 'NULL') . ', `customer`=' . (($data['customer'] !== '' && $data['customer'] !== NULL) ? "'{$data['customer']}'" : 'NULL') . ', `TOT`=' . (($data['TOT'] !== '' && $data['TOT'] !== NULL) ? "'{$data['TOT']}'" : 'NULL') . ', `MONTH`=' . (($data['MONTH'] !== '' && $data['MONTH'] !== NULL) ? "'{$data['MONTH']}'" : 'NULL') . ', `YEAR`=' . (($data['YEAR'] !== '' && $data['YEAR'] !== NULL) ? "'{$data['YEAR']}'" : 'NULL') . ', `DOCs`=' . (($data['DOCs'] !== '' && $data['DOCs'] !== NULL) ? "'{$data['DOCs']}'" : 'NULL') . ', `related`=' . (($data['related'] !== '' && $data['related'] !== NULL) ? "'{$data['related']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"_ordersSummary_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}

	$recID = db_insert_id(db_link());

	// hook: _ordersSummary_after_insert
	if(function_exists('_ordersSummary_after_insert')){
		$res = sql("select * from `_ordersSummary` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=array();
		if(!_ordersSummary_after_insert($data, getMemberInfo(), $args)){ return $recID; }
	}

	// mm: save ownership data
	set_record_owner('_ordersSummary', $recID, getLoggedMemberID());

	return $recID;
}

function _ordersSummary_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('_ordersSummary');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='_ordersSummary' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='_ordersSummary' and pkValue='$selected_id'");
	...
	// hook: _ordersSummary_before_delete
	if(function_exists('_ordersSummary_before_delete')){
		$args=array();
		if(!_ordersSummary_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	sql("delete from `_ordersSummary` where `id`='$selected_id'", $eo);

	// hook: _ordersSummary_after_delete
	if(function_exists('_ordersSummary_after_delete')){
		$args=array();
		_ordersSummary_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='_ordersSummary' and pkValue='$selected_id'", $eo);
}

function _ordersSummary_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('_ordersSummary');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='_ordersSummary' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='_ordersSummary' and pkValue='".makeSafe($selected_id)."'");
	...
	// hook: _ordersSummary_before_update
	if(function_exists('_ordersSummary_before_update')){
		$args=array();
		if(!_ordersSummary_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `_ordersSummary` set       `kind`=' . (($data['kind'] !== '' && $data['kind'] !== NULL) ? "'{$data['kind']}'" : 'NULL') . ', `company`=' . (($data['company'] !== '' && $data['company'] !== NULL) ? "'{$data['company']}'" : 'NULL') . ', `typedoc`=' . (($data['typedoc'] !== '' && $data['typedoc'] !== NULL) ? "'{$data['typedoc']}'" : 'NULL') . ', `customer`=' . (($data['customer'] !== '' && $data['customer'] !== NULL) ? "'{$data['customer']}'" : 'NULL') . ', `TOT`=' . (($data['TOT'] !== '' && $data['TOT'] !== NULL) ? "'{$data['TOT']}'" : 'NULL') . ', `MONTH`=' . (($data['MONTH'] !== '' && $data['MONTH'] !== NULL) ? "'{$data['MONTH']}'" : 'NULL') . ', `YEAR`=' . (($data['YEAR'] !== '' && $data['YEAR'] !== NULL) ? "'{$data['YEAR']}'" : 'NULL') . ', `DOCs`=' . (($data['DOCs'] !== '' && $data['DOCs'] !== NULL) ? "'{$data['DOCs']}'" : 'NULL') . ', `related`=' . (($data['related'] !== '' && $data['related'] !== NULL) ? "'{$data['related']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
		echo '<a href="_ordersSummary_view.php?SelectedID='.urlencode($selected_id)."\">{$Translation['< back']}</a>";
		exit;
	}


	// hook: _ordersSummary_after_update
	if(function_exists('_ordersSummary_after_update')){
		$res = sql("SELECT * FROM `_ordersSummary` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['id'];
		$args = array();
		if(!_ordersSummary_after_update($data, getMemberInfo(), $args)){ return; }
	}

	// mm: update ownership data
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='_ordersSummary' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function _ordersSummary_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		$res = sql("select * from `_ordersSummary` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], '_ordersSummary_view.php', false);
		}
			...
			$j("#kind-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_kind__RAND__.value, t: '_ordersSummary', f: 'kind' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: '_ordersSummary', f: 'kind' }; },
			...
			$j("#company-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_company__RAND__.value, t: '_ordersSummary', f: 'company' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: '_ordersSummary', f: 'company' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			$j("#typedoc-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_typedoc__RAND__.value, t: '_ordersSummary', f: 'typedoc' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: '_ordersSummary', f: 'typedoc' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			$j("#customer-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_customer__RAND__.value, t: '_ordersSummary', f: 'customer' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: '_ordersSummary', f: 'customer' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			$j("#related-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_related__RAND__.value, t: '_ordersSummary', f: 'related' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: '_ordersSummary', f: 'related' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/_ordersSummary_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/_ordersSummary_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return _ordersSummary_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return _ordersSummary_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return _ordersSummary_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$rdata = $jdata = get_defaults('_ordersSummary');
	if($selected_id){
		$jdata = get_joined_record('_ordersSummary', $selected_id);
		if($jdata === false) $jdata = get_defaults('_ordersSummary');
		$rdata = $row;
	}
	$templateCode .= loadView('_ordersSummary-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: _ordersSummary_dv
	if(function_exists('_ordersSummary_dv')){
		$args=array();
		_ordersSummary_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
