...
<div class="page-header row">
	...
	<h1><a href="./index.php">Projects</a> > <a href="./project.php?axp=<?php echo $_GET['axp']; ?>"><?php echo substr( $projectFile , 0 , strrpos( $projectFile , ".")); ?></a> > <a href="./output-folder.php?axp=<?php echo $_GET['axp'] ?>">  Select output folder</a> > Enabling MPI
	</h1>
...
<?php
	...
	$mpi_class->copy_file($source_class, $dest_class, true);
		...
		//includes
//		includeNeededParts($fileContent , $includesArray, $mpi_class);
...
//				
//                $mpi_class->progress_log->add("Installed search page to 'hooks/{$tableName}.php'.", 'text-success spacer');
//                $error = $mpi_class->last_error();
//
//                $mpi_class->progress_log->add("Skipped installing to 'hooks/{$tableName}.php' as a search page is already installed.", 'text-warning spacer');
//                $mpi_class->progress_log->add("Failed to install search page to 'hooks/{$tableName}.php': {$error}", 'text-danger spacer');
//                $mpi_class->progress_log->add($install_instructions, 'spacer');
//                $mpi_class->progress_log->add("<b>Error: Couldn't save 'hooks/{$fileName}': Check the permissions.</b>", 'text-danger spacer');
//	
