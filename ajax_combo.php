	...
	$lookups = array(   
		...
		'companies' => array(   
			...
			'codiceDestinatario' => array(
				...
				'parent_from' => '`kinds` ',
				'filterers' => array(),
				'custom_query' => 'SELECT `kinds`.`code`, IF(CHAR_LENGTH(`kinds`.`code`) || CHAR_LENGTH(`kinds`.`name`), CONCAT_WS(\'\', `kinds`.`code`, \' - \', `kinds`.`name`), \'\') FROM `kinds` WHERE `kinds`.`entity` LIKE \'%sdi-CodiceDestinatario%\' ORDER BY 2 ',
				'inherit_permissions' => false,
				'list_type' => 0,
				'not_null' => false
			),
