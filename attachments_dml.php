...
// Data functions (insert, update, delete, form) for table attachments

// This script and data application were generated by AppGini 5.72
// Download AppGini for free from https://bigprof.com/appgini/download/

function attachments_insert(){
	global $Translation;

	// mm: can member insert record?
	$arrPerm=getTablePermissions('attachments');
	if(!$arrPerm[1]){
		return false;
	}
	...
	$data['thumbUse'] = makeSafe($_REQUEST['thumbUse']);
		if($data['thumbUse'] == empty_lookup_value){ $data['thumbUse'] = ''; }
	$data['file'] = PrepareUploadedFile('file', 102398976,'jpg|jpeg|gif|png', false, '');

	/* for empty upload fields, when saving a copy of an existing record, copy the original upload field */
	if($_REQUEST['SelectedID']){
		$res = sql("select * from attachments where id='" . makeSafe($_REQUEST['SelectedID']) . "'", $eo);
		if($row = db_fetch_assoc($res)){
			if(!$data['file']) $data['file'] = makeSafe($row['file']);
		}
	}

	// hook: attachments_before_insert
	if(function_exists('attachments_before_insert')){
		$args=array();
		if(!attachments_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o = array('silentErrors' => true);
	sql('insert into `attachments` set       `name`=' . (($data['name'] !== '' && $data['name'] !== NULL) ? "'{$data['name']}'" : 'NULL') . ', ' . ($data['file'] != '' ? "`file`='{$data['file']}'" : '`file`=NULL') . ', `thumbUse`=' . (($data['thumbUse'] !== '' && $data['thumbUse'] !== NULL) ? "'{$data['thumbUse']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"attachments_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}
	...
	if($_REQUEST['filterer_contact']){
		sql("update `attachments` set `contact`='" . makeSafe($_REQUEST['filterer_contact']) . "' where `id`='" . makeSafe($recID, false) . "'", $eo);
	}

	// automatic company
	if($_REQUEST['filterer_company']){
		sql("update `attachments` set `company`='" . makeSafe($_REQUEST['filterer_company']) . "' where `id`='" . makeSafe($recID, false) . "'", $eo);
	}

	// hook: attachments_after_insert
	if(function_exists('attachments_after_insert')){
		$res = sql("select * from `attachments` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=array();
		if(!attachments_after_insert($data, getMemberInfo(), $args)){ return $recID; }
	}

	// mm: save ownership data
	set_record_owner('attachments', $recID, getLoggedMemberID());

	return $recID;
}

function attachments_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('attachments');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='attachments' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='attachments' and pkValue='$selected_id'");
	...
	// hook: attachments_before_delete
	if(function_exists('attachments_before_delete')){
		$args=array();
		if(!attachments_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	// delete file stored in the 'file' field
	$res = sql("select `file` from `attachments` where `id`='$selected_id'", $eo);
	...
	sql("delete from `attachments` where `id`='$selected_id'", $eo);

	// hook: attachments_after_delete
	if(function_exists('attachments_after_delete')){
		$args=array();
		attachments_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='attachments' and pkValue='$selected_id'", $eo);
}

function attachments_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('attachments');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='attachments' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='attachments' and pkValue='".makeSafe($selected_id)."'");
	...
	if($_REQUEST['file_remove'] == 1){
		...
		$res = sql("select `file` from `attachments` where `id`='".makeSafe($selected_id)."'", $eo);
		if($row=@db_fetch_row($res)){
			if($row[0]!=''){
				@unlink(getUploadDir('').$row[0]);
			}
	...
	// hook: attachments_before_update
	if(function_exists('attachments_before_update')){
		$args=array();
		if(!attachments_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `attachments` set       `name`=' . (($data['name'] !== '' && $data['name'] !== NULL) ? "'{$data['name']}'" : 'NULL') . ', ' . ($data['file']!='' ? "`file`='{$data['file']}'" : ($_REQUEST['file_remove'] != 1 ? '`file`=`file`' : '`file`=NULL')) . ', `thumbUse`=' . (($data['thumbUse'] !== '' && $data['thumbUse'] !== NULL) ? "'{$data['thumbUse']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
		echo '<a href="attachments_view.php?SelectedID='.urlencode($selected_id)."\">{$Translation['< back']}</a>";
		exit;
	}


	// hook: attachments_after_update
	if(function_exists('attachments_after_update')){
		$res = sql("SELECT * FROM `attachments` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['id'];
		$args = array();
		if(!attachments_after_update($data, getMemberInfo(), $args)){ return; }
	}

	// mm: update ownership data
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='attachments' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function attachments_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		$res = sql("select * from `attachments` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'attachments_view.php', false);
		}
			...
			$j("#contact-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_contact__RAND__.value, t: 'attachments', f: 'contact' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'attachments', f: 'contact' }; },
			...
			$j("#company-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_company__RAND__.value, t: 'attachments', f: 'company' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'attachments', f: 'company' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/attachments_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/attachments_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}

	// process form title
	$templateCode = str_replace('<%%DETAIL_VIEW_TITLE%%>', 'Attache details', $templateCode);
	$templateCode = str_replace('<%%RND1%%>', $rnd1, $templateCode);
	$templateCode = str_replace('<%%EMBEDDED%%>', ($_REQUEST['Embedded'] ? 'Embedded=1' : ''), $templateCode);
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return attachments_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return attachments_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	$rdata = $jdata = get_defaults('attachments');
	if($selected_id){
		$jdata = get_joined_record('attachments', $selected_id);
		if($jdata === false) $jdata = get_defaults('attachments');
		$rdata = $row;
	}
	$templateCode .= loadView('attachments-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: attachments_dv
	if(function_exists('attachments_dv')){
		$args=array();
		attachments_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
