...
if (!$table_from) {
    exit(error_message("Access denied to $table_name!",'', false));
}
$where_id = intval($_REQUEST['id']);

if (!$where_id) {
    exit(error_message("The monkey are eating the $table_name Code. (the $table_name code are lost)",'', true));
}

/* retrive from table info */
...
if (!($result = db_fetch_assoc($res))) {
    exit(error_message("$table_name not found",'', false));
}
...
$res= sql("SELECT {$table_fields} FROM {$table_from} AND `addresses`.`contact` = {$where_id} AND `addresses`.`default` = 1",$eo);

if (!($address = db_fetch_assoc($res))) {
    //if not setting a default address, get the first.
    $res= sql("SELECT {$table_fields} FROM {$table_from} AND `addresses`.`contact` = {$where_id} order by `addresses`.id ASC LIMIT 1",$eo);
    if (!($address = db_fetch_assoc($res))) {
...
<?php } ?>
    <div class="small-box bg-aqua">
            ...
            <a id="companies_view_parent" pt="<?php echo $table_name; ?>" myid="<?php echo $where_id; ?>" class="btn btn-sm view_parent" type="button" title="Contact Details" onclick="showParent(this);" >more info
