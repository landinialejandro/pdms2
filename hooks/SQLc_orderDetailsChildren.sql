select     `ordersDetails`.`id` as 'id',
    ...
    IF(    CHAR_LENGTH(`kinds2`.`value`), CONCAT(FORMAT(`kinds2`.`value`,2), ' %'), '') as 'TAX',
    IF(    CHAR_LENGTH(`products1`.`productCode`) || CHAR_LENGTH(`products1`.`id`), 
    CONCAT_WS('',   `products1`.`productCode`, '-', `products1`.`id`), '') as 'batch',
