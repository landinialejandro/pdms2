...
function kinds_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$res = sql("select `code` from `kinds` where `code`='$selected_id'", $eo);
	$code = db_fetch_row($res);
	$rires = sql("select count(1) from `_ordersSummary` where `kind`='".addslashes($code[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `code` from `kinds` where `code`='$selected_id'", $eo);
	$code = db_fetch_row($res);
	$rires = sql("select count(1) from `_ordersSummary` where `typedoc`='".addslashes($code[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `code` from `kinds` where `code`='$selected_id'", $eo);
	$code = db_fetch_row($res);
	$rires = sql("select count(1) from `companies` where `modalitaPagamento`='".addslashes($code[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `code` from `kinds` where `code`='$selected_id'", $eo);
	$code = db_fetch_row($res);
	$rires = sql("select count(1) from `companies` where `FormatoTrasmissione`='".addslashes($code[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$rires = sql("select count(1) from `contacts` where `kind`='".addslashes($name[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
...
function kinds_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if(is_file(dirname(__FILE__).'/hooks/kinds.entity.csv')){
		...
		$combo_entity->ListItem = explode('||', entitiesToUTF8(convertLegacyOptions("Contacts;;check_csv")));
		$combo_entity->ListData = $combo_entity->ListItem;
	}
