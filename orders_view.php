...
// Download AppGini for free from https://bigprof.com/appgini/download/
	...
	$x->QueryFieldsTV = array(   
		...
		"IF(    CHAR_LENGTH(`companies1`.`companyCode`) || CHAR_LENGTH(`companies1`.`companyName`), CONCAT_WS('',   `companies1`.`companyCode`, ' - ', `companies1`.`companyName`), '') /* Azienda */" => "company",
		...
		"`orders`.`importoSconto`" => "importoSconto",
		"`orders`.`orderTotal`" => "orderTotal",
		"`orders`.`RIT_importoRitenuta`" => "RIT_importoRitenuta",
	...
	$x->SortFields = array(   
		...
		22 => '`orders`.`trust`',
		23 => '`orders`.`overdraft`',
		24 => '`orders`.`commisionFee`',
		25 => '`orders`.`commisionRate`',
		26 => '`orders`.`consigneeHour`',
		27 => 27,
		28 => '`orders`.`related`',
		29 => 29
	);
	...
	$x->QueryFieldsFilters = array(   
		...
		"IF(    CHAR_LENGTH(`companies1`.`companyCode`) || CHAR_LENGTH(`companies1`.`companyName`), CONCAT_WS('',   `companies1`.`companyCode`, ' - ', `companies1`.`companyName`), '') /* Azienda */" => "Azienda",
		...
		"`orders`.`importoSconto`" => "Importo Sconto o Mag PA",
		"`orders`.`orderTotal`" => "Importo Totale doc",
		"`orders`.`RIT_importoRitenuta`" => "RIT importo",
	...
	$x->ColWidth   = array(  150, 150, 150, 150, 150, 150, 200, 150, 150, 150, 150, 150, 150, 150, 150, 150, 150, 150, 150, 150, 150, 150);
	$x->ColCaption = array("Tipo ordine", "Azienda", "Documento", "Numero", "Divisa", "Causale", "Cliente", "Supplier", "Data Ordine", "Data Ora Ritiro PA", "Data di spedizione", "Spedizione a mezzo", "Pallets", "Targa Automezzo", "Importo Sconto o Mag PA", "Importo Totale doc", "RIT importo", "Credito o cassa", "Provvigione su ordine", "CommisionRate", "Ora di consegna", "Luogo consegna");
	$x->ColFieldName = array('kind', 'company', 'typeDoc', 'multiOrder', 'divisa', 'causale', 'customer', 'supplier', 'date', 'dateRequired', 'dateShipped', 'shipVia', 'pallets', 'licencePlate', 'importoSconto', 'orderTotal', 'RIT_importoRitenuta', 'cashCredit', 'commisionFee', 'commisionRate', 'consigneeHour', 'consigneePlace');
	$x->ColNumber  = array(2, 3, 4, 5, 6, 7, 8, 9, 11, 12, 13, 14, 16, 17, 18, 19, 20, 21, 24, 25, 26, 27);
	...
	if(strpos($x->HTML, '<!-- tv data below -->')){
		...
		if($row = db_fetch_row($res)){
			...
			$sumRow .= '<td class="orders-divisa"></td>';
			$sumRow .= '<td class="orders-causale"></td>';
			$sumRow .= '<td class="orders-customer"></td>';
			$sumRow .= '<td class="orders-supplier"></td>';
			$sumRow .= '<td class="orders-date"></td>';
			$sumRow .= '<td class="orders-dateRequired"></td>';
			$sumRow .= '<td class="orders-dateShipped"></td>';
			...
			$sumRow .= '<td class="orders-importoSconto"></td>';
			$sumRow .= "<td class=\"orders-orderTotal text-right\">{$row[1]}</td>";
			$sumRow .= '<td class="orders-RIT_importoRitenuta"></td>';
